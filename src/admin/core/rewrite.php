...
class rewrite{
   ...
   public static function note($in){

        if(empty($in)){
            return "";
        }
        return self::getRewrite($in);
    }

  public static function getRewrite($name = "", $table = "", $id = "-"){
       ...
       foreach ($arrRuFromB as $key => $val)
           $tr[$val] = $arrRuToS[$key];
       foreach ($arrRuFromS as $key => $val)
           $tr[$val] = $arrRuToS[$key];
       ...
       */
		...
		if( strlen( $out ) == 0 ) $out = "none-".md5(microtime());
		$out = strtolower( $out );
