...
class fileupload{
    ...
    public function __construct(){
        ...
        if (in_array($extension, $allowedExts)) {
            ...
            if(file_exists(dirname(__FILE__)."/../../files/".str_replace(".".$extension, "", rewrite::getRewrite($jmeno)). "." . $extension)){
           ...
           // echo_array($_FILES["file"]["tmp_name"]);

            copy($_FILES["file"]["tmp_name"], dirname(__FILE__)."/../../files/".$name);


            if($extension == "png" && $rocket["convertPNGtoJPG"] == true){
                $images = dirname(__FILE__)."/../../files/".$name;
            ...
            if($extension == "jpg" or $extension == "jpeg") {
                ...
                ImageJPEG($images_fin,dirname(__FILE__)."/../../files/big-".$name);
                ...
                if ($_POST["addData"]=="noresize"){
                    ...
                    $images_fin = ImageCreateTrueColor($width, $height);
                    ImageCopyResampled($images_fin, $images_orig, 0, 0, 0, 0, $width+1, $height+1, $photoX, $photoY);
                    ImageJPEG($images_fin,dirname(__FILE__)."/../../files/".$name);
                    ImageDestroy($images_orig);
                    ImageDestroy($images_fin);
                }
                ...
                $images_fin = ImageCreateTrueColor($width, $height);
                ImageCopyResampled($images_fin, $images_orig, 0, 0, 0, 0, $width+1, $height+1, $photoX, $photoY);
                ImageJPEG($images_fin,dirname(__FILE__)."/../../files/wm-".$name);
                ImageDestroy($images_orig);
                ImageDestroy($images_fin);
                ...
                if ($rocket["watermarkForFullSize"]){
                    $input_name = dirname(__FILE__)."/../../files/wm-".$name;
                ...
                $images_fin = ImageCreateTrueColor($width, $height);
                ImageCopyResampled($images_fin, $images_orig, 0, 0, 0, 0, $width+1, $height+1, $photoX, $photoY);
                ImageJPEG($images_fin,dirname(__FILE__)."/../../files/small-".$name);
                ImageDestroy($images_orig);
                ImageDestroy($images_fin);
                ...
                $images_fin = ImageCreateTrueColor($width, $height);
                ImageCopyResampled($images_fin, $images_orig, 0, 0, 0, 0, $width+1, $height+1, $photoX, $photoY);
                ImageJPEG($images_fin,dirname(__FILE__)."/../../files/medium-".$name);
                ImageDestroy($images_orig);
                ImageDestroy($images_fin);
                $zmensoval = "ano";
            ...
            if (in_array($extension, $fotoExts)){
                $templink =  dirname(__FILE__)."/../../files/" . $name;
            }else{
                $templink =  "img/no.png";
            }

            $typ = "Neznámý";
            if (strtolower($extension) == "gif") {$typ = "obr";         }
            if (strtolower($extension) == "jpeg") {$typ = "obr";        }
            if (strtolower($extension) == "jpg") {$typ = "obr";         }
            if (strtolower($extension) == "png") {$typ = "obr";         }
